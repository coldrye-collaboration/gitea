...
func TestCreateIssueDependency(t *testing.T) {
	...
	err = issues_model.CreateIssueDependency(db.DefaultContext, user1, issue1, issue2, issues_model.DependencyTypeBlockedBy)
	assert.NoError(t, err)
	...
	err = issues_model.CreateIssueDependency(db.DefaultContext, user1, issue2, issue1, issues_model.DependencyTypeBlockedBy)
	assert.Error(t, err)
